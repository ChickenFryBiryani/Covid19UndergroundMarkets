...
class MySQLcryptomarketsDB:
    ...
    def CheckWhetherScrapingProductDescription(self, sProductMarketID):
        # Need to use (nMarketGlobalID, sProductMarketID) to determine whether we need to
        # scrape this page (product description) or not
        # The rule is that if this page has been scraped 30 (m_nScrapingFreqDaysProductDesc) days ago,
        # we scrape it again. Otherwise, skip.
        ...
        aUpdateStmt_product_list = "UPDATE product_list SET my_lock_pd=1 WHERE cryptomarket_global_ID='" + \
                                   str(self.m_nMarketGlobalID) + "' AND product_market_ID='" + \
                                   sProductMarketID + "' AND my_lock_pd = 0;"
        ...
        if nNumOfAffectedRows == 1:     # the product is already in the product_list, check the last scraping time
            # it means 1 record is updated, we lock it.
            aSelectStmt_product_list = "SELECT product_global_ID, last_scraping_time_pd FROM product_list WHERE " \
                                       "cryptomarket_global_ID = '" + str(self.m_nMarketGlobalID) + \
            ...
            if nDurationDays < self.m_nScrapingFreqDaysProductDesc:     # last scraping date is within 30 days,
                bWhetherScraping = False    # so no need to scrape the web page again.
        else:   # nNumOfAffectedRows == 0, the product is 1. not in the product_list 2. being checked by other program
            # check whether this product exists or not in the product_list table:
            aSelectStmt_product_list = "SELECT product_global_ID FROM product_list WHERE cryptomarket_global_ID = '" + \
                                       str(self.m_nMarketGlobalID) + "' AND product_market_ID = '" + \
                                       sProductMarketID + "';"
            aCursorDB_cryptomarkets.execute(aSelectStmt_product_list)
            if aCursorDB_cryptomarkets.rowcount == 0:   # this product does not exist,Insert this product into the table
                aInsertStmt_product_list = ("INSERT INTO product_list (cryptomarket_global_ID, product_market_ID, "
                                            ...
                                            "%(last_scraping_time_pd)s, %(my_lock_pd)s, %(last_scraping_time_pr)s, "
                                            "%(my_lock_pr)s)")
                aData_product_list = {
                ...
                aSelectStmt_product_list = "SELECT product_global_ID FROM product_list WHERE " \
                                           "cryptomarket_global_ID = '" + str(self.m_nMarketGlobalID) +\
                ...
                bWhetherScraping = False    # This product exists in product_list; my_lock_pd = 1 :
                # being checked by other scrapers or left as 1 by previous scrapers.
        aMariaDB_cryptomarkets.close()
        return bWhetherScraping

    def UpdateDatabaseUploadFileProductDescription(self, sLocalOutputFileName, sLocalOutputFileNameFullPath,
                                                   sProductMarketID):
        # Upload the local file to the sever
        sSCP_Command = "sshpass -p '{}' scp {} {}@{}:{}".format(self.m_sServerPasswd, sLocalOutputFileNameFullPath,
        ...
        aMariaDB_cryptomarkets = mysql.connector.connect(host=self.m_sServerHost, user=self.m_sServerDatabaseUser,
                                                         ...
                                                         buffered=self.m_bServerSQLBuffered)
        aCursorDB_cryptomarkets = aMariaDB_cryptomarkets.cursor(dictionary=True)
        aInsertStmt_product_scraping_event = ("INSERT INTO product_desc_scraping_event (product_global_ID, "
                                              "scraping_time, product_desc_file_path_in_FS, student_ID) "
                                              "VALUES (%(product_global_ID)s, %(scraping_time)s, "
                                              "%(product_desc_file_path_in_FS)s, %(student_ID)s)")
        ...
        aUpdateStmt_product_list = "UPDATE product_list SET my_lock_pd = 0, last_scraping_time_pd = '" + \
                                   self.m_sCurrentUTCTime + "' WHERE cryptomarket_global_ID = '" + \
                                   str(self.m_nMarketGlobalID) + "' AND product_market_ID = '" + \
                                   sProductMarketID + "' AND my_lock_pd = 1;"
        aCursorDB_cryptomarkets.execute(aUpdateStmt_product_list)
        aMariaDB_cryptomarkets.commit()
        aMariaDB_cryptomarkets.close()
    ...
    def CheckWhetherScrapingProductRating(self, sProductMarketID):
        # Need to use (nMarketGlobalID, sProductMarketID) to determine whether we need to
        # scrape this page (product rating) or not
        # The rule is that if this page has been scraped 15 (m_nScrapingFreqDaysProductRating) days ago,
        # we scrape it again. Otherwise, skip.
        bWhetherScraping = True     # the initial status is to scrape it
        # query basic information of the mysql database
        aMariaDB_cryptomarkets = mysql.connector.connect(host=self.m_sServerHost, user=self.m_sServerDatabaseUser,
        ...
        aCursorDB_cryptomarkets = aMariaDB_cryptomarkets.cursor(dictionary=True)
        # Fetch product information
        aUpdateStmt_product_list = "UPDATE product_list SET my_lock_pr = 1 WHERE cryptomarket_global_ID = '{}' " \
                                   "AND product_market_ID = '{}' AND my_lock_pr = 0;".format(self.m_nMarketGlobalID,
                                                                                             sProductMarketID)
        aCursorDB_cryptomarkets.execute(aUpdateStmt_product_list)
        nNumOfAffectedRows = aCursorDB_cryptomarkets.rowcount
        aMariaDB_cryptomarkets.commit()
        if nNumOfAffectedRows == 1:     # the product is already in the product_list, Check the last scraping time
            # it means 1 record is updated, we lock it.
            aSelectStmt_product_list = "SELECT product_global_ID, last_scraping_time_pr FROM product_list WHERE " \
                                       "cryptomarket_global_ID = '{}' AND product_market_ID = '{}' AND " \
                                       "my_lock_pr = 1;".format(self.m_nMarketGlobalID, sProductMarketID)
            aCursorDB_cryptomarkets.execute(aSelectStmt_product_list)
            aOneProductRecord = aCursorDB_cryptomarkets.fetchone()
            self.m_nProductGlobalID = aOneProductRecord["product_global_ID"]
            ...
            aDuration = aCurrentUTCTime - aLastScrapingUTCTime # if the time difference is larger than 7 days
            nDurationDays = aDuration.days
            if nDurationDays < self.m_nScrapingFreqDaysProductRating:   # last scraping date is within 7 days
                bWhetherScraping = False    # so no need to scrape the web page again.
        else:   # nNumOfAffectedRows == 0, the product is 1. not in the product_list 2. being checked by other program
            # check whether this product exists or not in the product_list table:
            aSelectStmt_product_list = "SELECT product_global_ID FROM product_list WHERE cryptomarket_global_ID='{}' " \
                                       "AND product_market_ID = '{}';".format(self.m_nMarketGlobalID, sProductMarketID)
            aCursorDB_cryptomarkets.execute(aSelectStmt_product_list)
            if aCursorDB_cryptomarkets.rowcount == 0:   # product does not exist, Insert this product into the table
                aInsertStmt_product_list = ("INSERT INTO product_list (cryptomarket_global_ID, product_market_ID, "
                                            "last_scraping_time_pd, my_lock_pd, last_scraping_time_pr, my_lock_pr) "
                                            "VALUES (%(cryptomarket_global_ID)s, %(product_market_ID)s, "
                                            "%(last_scraping_time_pd)s, %(my_lock_pd)s, %(last_scraping_time_pr)s, "
                ...
                # need to get the product_global_ID
                aSelectStmt_product_list = "SELECT product_global_ID FROM product_list WHERE cryptomarket_global_ID=" \
                                           "'{}' AND product_market_ID = '{}';".format(self.m_nMarketGlobalID,
                                                                                       sProductMarketID)
                aCursorDB_cryptomarkets.execute(aSelectStmt_product_list)
                aOneProductRecord = aCursorDB_cryptomarkets.fetchone()
                self.m_nProductGlobalID = aOneProductRecord["product_global_ID"]
            else:   # aCursorDB_cryptomarkets.rowcount == 1:
                # This product exists in product_list; my_lock_pr = 1
                # being checked by other scrapers or left as 1 by previous scrapers.
                bWhetherScraping = False
        aMariaDB_cryptomarkets.close()
        return bWhetherScraping

    def UpdateDatabaseUploadFileProductRating(self, sLocalOutputFileName, sLocalOutputFileNameFullPath,
                                              sProductMarketID):
        # Upload the local file to the sever
        sSCP_Command = "sshpass -p '{}' scp {} {}@{}:{}".format(self.m_sServerPasswd, sLocalOutputFileNameFullPath,
                                                                self.m_sServerUser, self.m_sServerHost,
        ...
        aInsertStmt_product_scraping_event = (
            "INSERT INTO product_rating_scraping_event (product_global_ID, scraping_time, "
            "product_rating_file_path_in_FS, student_ID) "
        ...
        aUpdateStmt_product_list = "UPDATE product_list SET my_lock_pr = 0, last_scraping_time_pr = '{}' " \
                                   "WHERE cryptomarket_global_ID = '{}' AND product_market_ID = '{}' AND " \
                                   "my_lock_pr = 1;".format(self.m_sCurrentUTCTime, self.m_nMarketGlobalID,
                                                            sProductMarketID)
        aCursorDB_cryptomarkets.execute(aUpdateStmt_product_list)
        aMariaDB_cryptomarkets.commit()
        aMariaDB_cryptomarkets.close()
    ...
    def CheckWhetherScrapingVendorProfile(self, sVendorMarketID):
        # Need to use (nMarketGlobalID, sVendorMarketID) to determine
        # whether we need to scrape this page (vendor profile) or not
        # The rule is that if this page has been scraped 30 (m_nScrapingFreqDaysVendorProfile) days ago,
        # we scrape it again. Otherwise, skip.
        bWhetherScraping = True     # the initial status is to scrape it
        # query basic information of the mysql database
        aMariaDB_cryptomarkets = mysql.connector.connect(host=self.m_sServerHost, user=self.m_sServerDatabaseUser,
        ...
        aUpdateStmt_vendor_list = "UPDATE vendor_list SET my_lock_vp = 1 WHERE cryptomarket_global_ID = '{}' " \
                                  "AND vendor_market_ID = '{}' AND my_lock_vp = 0;".format(self.m_nMarketGlobalID,
                                                                                           sVendorMarketID)
        aCursorDB_cryptomarkets.execute(aUpdateStmt_vendor_list)
        nNumOfAffectedRows = aCursorDB_cryptomarkets.rowcount
        aMariaDB_cryptomarkets.commit()
        if nNumOfAffectedRows == 1:     # the vendor is already in the vendor_list, Check the last scraping time
            # it means 1 record is updated, we lock it.
            aSelectStmt_vendor_list = "SELECT vendor_global_ID, last_scraping_time_vp FROM vendor_list " \
                                      "WHERE cryptomarket_global_ID = '{}' AND vendor_market_ID = '{}' " \
                                      "AND my_lock_vp = 1;".format(self.m_nMarketGlobalID, sVendorMarketID)
            ...
            nDurationDays = aDuration.days
            if nDurationDays < self.m_nScrapingFreqDaysVendorProfile:   # last scraping date is within 30 days,
                bWhetherScraping = False    # so no need to scrape the web page again.
        else:   # nNumOfAffectedRows == 0, the vendor is 1. not in the vendor_list or 2. being checked by other program
            # check whether this vendor exists or not in the vendor_list table:
            aSelectStmt_vendor_list = "SELECT vendor_global_ID FROM vendor_list WHERE cryptomarket_global_ID = '{}' " \
                                      "AND vendor_market_ID = '{}';".format(self.m_nMarketGlobalID, sVendorMarketID)
            aCursorDB_cryptomarkets.execute(aSelectStmt_vendor_list)
            if aCursorDB_cryptomarkets.rowcount == 0:   # this vendor does not exist, Insert this vendor into the table
                aInsertStmt_vendor_list = ("INSERT INTO vendor_list (cryptomarket_global_ID, vendor_market_ID, "
                                           ...
                                           "%(last_scraping_time_vp)s, %(my_lock_vp)s, %(last_scraping_time_vr)s, "
                                           "%(my_lock_vr)s)")
                aData_vendor_list = {
                    'cryptomarket_global_ID': self.m_nMarketGlobalID,
                ...
                aSelectStmt_vendor_list = "SELECT vendor_global_ID FROM vendor_list " \
                                          "WHERE cryptomarket_global_ID = '{}' " \
                                          "AND vendor_market_ID = '{}';".format(self.m_nMarketGlobalID, sVendorMarketID)
                aCursorDB_cryptomarkets.execute(aSelectStmt_vendor_list)
                aOneVendorRecord = aCursorDB_cryptomarkets.fetchone()
                self.m_nVendorGlobalID = aOneVendorRecord["vendor_global_ID"]
    ...
    def UpdateDatabaseUploadFileVendorProfile(self, sLocalOutputFileName, sLocalOutputFileNameFullPath,
                                              sVendorMarketID):
        # Upload the local file to the sever
        ...
        aInsertStmt_vendor_scraping_event = (
            "INSERT INTO vendor_profile_scraping_event (vendor_global_ID, scraping_time, "
            "vendor_profile_file_path_in_FS, student_ID) "
        ...
        aUpdateStmt_vendor_list = "UPDATE vendor_list SET my_lock_vp = 0, last_scraping_time_vp = '{}' " \
                                  "WHERE cryptomarket_global_ID = '{}' AND vendor_market_ID = '{}' " \
                                  "AND my_lock_vp = 1;".format(self.m_sCurrentUTCTime, self.m_nMarketGlobalID,
                                                               sVendorMarketID)
        aCursorDB_cryptomarkets.execute(aUpdateStmt_vendor_list)
        aMariaDB_cryptomarkets.commit()
        aMariaDB_cryptomarkets.close()
    ...
    def CheckWhetherScrapingVendorRating(self, sVendorMarketID):
        # Need to use (nMarketGlobalID, sVendorMarketID) to determine whether
        # we need to scrape this page (vendor rating) or not
        # The rule is that if this page has been scraped 7 (m_nScrapingFreqDaysVendorRating) days ago,
        # we scrape it again. Otherwise, skip.
        bWhetherScraping = True     # the initial status is to scrape it
        # query basic information of the mysql database
        aMariaDB_cryptomarkets = mysql.connector.connect(host=self.m_sServerHost, user=self.m_sServerDatabaseUser,
        ...
        aCursorDB_cryptomarkets = aMariaDB_cryptomarkets.cursor(dictionary=True)
        # Fetch vendor information
        aUpdateStmt_vendor_list = "UPDATE vendor_list SET my_lock_vr = 1 WHERE cryptomarket_global_ID = '{}' " \
                                  "AND vendor_market_ID = '{}' AND my_lock_vr = 0;".format(self.m_nMarketGlobalID,
                                                                                           sVendorMarketID)
        aCursorDB_cryptomarkets.execute(aUpdateStmt_vendor_list)
        nNumOfAffectedRows = aCursorDB_cryptomarkets.rowcount
        aMariaDB_cryptomarkets.commit()
        if nNumOfAffectedRows == 1:     # the vendor is already in the vendor_list db, Check the last scraping time
            # it means 1 record is updated, we lock it.
            aSelectStmt_vendor_list = "SELECT vendor_global_ID, last_scraping_time_vr FROM vendor_list " \
                                      "WHERE cryptomarket_global_ID = '{}' AND vendor_market_ID = '{}' " \
                                      "AND my_lock_vr = 1;".format(self.m_nMarketGlobalID, sVendorMarketID)
            aCursorDB_cryptomarkets.execute(aSelectStmt_vendor_list)
            aOneVendorRecord = aCursorDB_cryptomarkets.fetchone()
            self.m_nVendorGlobalID = aOneVendorRecord["vendor_global_ID"]
            ...
            aDuration = aCurrentUTCTime - aLastScrapingUTCTime # if the time difference is larger than 7 days
            nDurationDays = aDuration.days
            if nDurationDays < self.m_nScrapingFreqDaysVendorRating:    # last scraping date is within 7 days,
                bWhetherScraping = False    # so no need to scrape the web page again.
        else:   # nNumOfAffectedRows == 0, the vendor is 1. not in the vendor_list or 2. being checked by other program
            # check whether this vendor exists or not in the vendor_list table:
            aSelectStmt_vendor_list = "SELECT vendor_global_ID FROM vendor_list WHERE cryptomarket_global_ID = '{}' " \
            ...
            if aCursorDB_cryptomarkets.rowcount == 0:   # this vendor does not exist, Insert this vendor into the table
                ...
                bWhetherScraping = False    # This vendor exists in vendor_list; my_lock_vr = 1 :
                # being checked by other scrapers or left as 1 by previous scrapers.
        aMariaDB_cryptomarkets.close()
        return bWhetherScraping
    ...
    def UpdateDatabaseUploadFileVendorRating(self, sLocalOutputFileName, sLocalOutputFileNameFullPath, sVendorMarketID):
        ...
        aCursorDB_cryptomarkets = aMariaDB_cryptomarkets.cursor(dictionary=True)
        aInsertStmt_vendor_scraping_event = (
            "INSERT INTO vendor_rating_scraping_event (vendor_global_ID, scraping_time, "
            "vendor_rating_file_path_in_FS, student_ID) "
        ...
        aUpdateStmt_vendor_list = "UPDATE vendor_list SET my_lock_vr = 0, last_scraping_time_vr = '{}' " \
                                  "WHERE cryptomarket_global_ID = '{}' AND vendor_market_ID = '{}' " \
                                  "AND my_lock_vr = 1;".format(self.m_sCurrentUTCTime, self.m_nMarketGlobalID,
                                                               sVendorMarketID)
        aCursorDB_cryptomarkets.execute(aUpdateStmt_vendor_list)
        aMariaDB_cryptomarkets.commit()
        aMariaDB_cryptomarkets.close()
    ...
    def AddToProductListIfNotExist(self, sProductMarketID):
        # Use (nMarketGlobalID, sProductMarketID) to determine whether
        # we need to add this product to table product_list or not
    ...
    def AddToVendorListIfNotExist(self, sVendorMarketID):
        # Use (nMarketGlobalID, sVendorMarketID) to determine whether
        # we need to add this vendor to table vendor_list or not
    ...
    def SelectNewProductMarketIDsToBeScraped(self):
        ...
        aLastScrapingTimeThreshold = datetime.datetime.utcnow() - \
                                     datetime.timedelta(days=self.m_nScrapingFreqDaysProductRating)
        sLastScrapingTimeThreshold = aLastScrapingTimeThreshold.strftime("%Y%m%d%H%M%S")
        aSelectStmt_product_list = "SELECT product_market_ID FROM product_list WHERE cryptomarket_global_ID = '{}' " \
                                   "AND last_scraping_time_pd < '{}' " \
                                   "ORDER BY last_scraping_time_pd ASC;".format(self.m_nMarketGlobalID,
                                                                                sLastScrapingTimeThreshold)
        aCursorDB_cryptomarkets.execute(aSelectStmt_product_list)
        aMariaDB_cryptomarkets.commit()
    ...
    def SelectNewVendorMarketIDsToBeScraped(self):
        ...
        aLastScrapingTimeThreshold = datetime.datetime.utcnow() - \
                                     datetime.timedelta(days=self.m_nScrapingFreqDaysVendorRating)
        sLastScrapingTimeThreshold = aLastScrapingTimeThreshold.strftime("%Y%m%d%H%M%S")
        aSelectStmt_vendor_list = "SELECT vendor_market_ID FROM vendor_list WHERE cryptomarket_global_ID = '{}' " \
                                  "AND last_scraping_time_vp < '{}' " \
                                  "ORDER BY last_scraping_time_vp ASC;".format(self.m_nMarketGlobalID,
                                                                               sLastScrapingTimeThreshold)
        aCursorDB_cryptomarkets.execute(aSelectStmt_vendor_list)
        aMariaDB_cryptomarkets.commit()
